...
description: 从视频/音频链接提取文字，默认输出简体中文逐字稿（完整保留全部原话、按说话顺序成段，不做整理成文），也支持直接解析本地 .md/.txt/.srt 文字稿（总结/解读/初筛/问答）及微信视频号/公众号等下载器难抓的来源。当用户粘贴音视频链接（B站、央视、百度、腾讯、YouTube、直链 mp4/mp3、微信视频号 weixin.qq.com/sph、公众号文章等）希望"转成字幕/文字稿/逐字稿/文章/报告"、要求对链接做语音转写、需要对已提取文字做总结/解读/初筛/问答、或检索转写知识库（~/Documents/字幕知识库）时使用。核心流程：(1) 优先提取内嵌/在线字幕轨；(2) 无字幕时回退 Whisper 语音识别；(3) 视频号等 yt-dlp 抓不到的按情况 E 从本地微信缓存捕获；(4) 统一繁体转简体、保守修正明显同音错字，输出完整逐字稿（仅当用户明确要求时才另出成文/总结/解读版）；(5) 结果自动存入本地知识库供调用。也适用于对已在本机的音视频文件直接处理。
---
...
**默认交付 = 逐字稿**：完整保留视频里说出的每一句话，按原顺序排成自然段（无时间轴、无改写、无提炼、无小标题），只做繁转简和对明显识别错误的保守修正。仅当用户明确说"整理成文 / 文章 / 报告 / 总结 / 解读"时，才在逐字稿之外另输出一版整理成文的文章。
...
4. 检查逐字稿是否**完整覆盖原话**（与 `transcript.txt` 对照，不能漏句）；确认后交给用户，另存 `.md`。用户明确要求"成文/文章/总结/解读"时，才额外产出整理版文章（此时逐字稿仍保留，作为附录或单独文件）。
...
- **默认输出 = 简体中文逐字稿**：完整保留全部原话，按说话顺序排成自然段（无时间轴、无小标题、无改写、无提炼），不是 srt 时间轴，也不是整理成文的文章。
- 只做繁体转简体与"确定无疑"的明显错字修正；不自行添加观点、不概括、不删句、不补衔接。
- 用户明确要求"成文 / 文章 / 报告 / 总结 / 解读"时才额外整理成文；**逐字稿始终是基础交付**。
