...
2. **补全中文标点**：转写/字幕文本往往整段无标点，先跑 `python3 <skill_dir>/scripts/punctuate.py <上步文本> <输出> --join` 规则打底（按行补 `。？！，` 并把相邻短句合并成自然段；字幕轨自带标点的行原样保留）。**只加标点，不增删内容**。若文本本身标点完整，跳过此步。
3. **逐段复核（标点 + 错字一起做）**：
   - 修标点：把规则漏判/误判处按语义改对（疑问句→`？`、感叹句→`！`、句尾该断句处→`。`、承接处→`，`），去掉明显多余的逗号。
   - 保守修正同音/近音错字：用**常识与上下文**修正，重点是**人名、地名、固定成语、专有词**。举例：`古墓`→`谷牧`、`坑槍`→`踉蹌`、`十一屆三中懸會`→`十一屆三中全會`、`波蘭壯闊`→`波瀾壯闊`、`中美美`→`钟美美`、`贺岗`→`鹤岗`、`博士大学`→`伯克利`。字幕轨的错字通常比语音识别少，但**自动生成字幕（auto-caption）也会有同音/乱序错误**，仍需此步。规则：**只有正确写法十分确定时才改**，保留原句措辞，不增删句子内容。
