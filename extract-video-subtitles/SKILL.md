...
description: 从视频/音频链接提取文字，默认输出简体中文逐字稿（即成文文章：完整保留全部原话、按说话顺序成段），并可就用户提问对逐字稿做解析/总结/初筛/问答，也支持直接解析本地 .md/.txt/.srt 文字稿及微信视频号/公众号等下载器难抓的来源。当用户粘贴音视频链接（B站、央视、百度、腾讯、YouTube、直链 mp4/mp3、微信视频号 weixin.qq.com/sph、公众号文章等）希望"转成字幕/文字稿/逐字稿/成文文章/报告"、要求对链接做语音转写、对已提取文字做总结/解读/初筛/问答、或检索转写知识库（~/Documents/字幕知识库）时使用。核心流程：(1) 优先提取内嵌/在线字幕轨；(2) 无字幕时回退 Whisper 语音识别；(3) 视频号等 yt-dlp 抓不到的按情况 E 从本地微信缓存捕获；(4) 统一繁体转简体、补全中文标点、保守修正明显同音错字，产出完整带标点逐字稿（即成文文章）；(5) 用户提问时基于逐字稿做解析/总结/问答；(6) 结果自动存入本地知识库供调用。也适用于对已在本机的音视频文件直接处理。
---

# 视频字幕提取

从音视频链接提取文字。**优先使用字幕轨**（在线字幕或内嵌字幕），只有确认**没有字幕**时才回退到 Whisper 语音识别。两条路线之后都做同样的后处理：繁转简 → 保守修正明显同音错字 → 按说话顺序整理为逐字稿段落。

**默认交付 = 简体中文逐字稿（即成文文章）**：完整保留视频里说出的每一句话，按原顺序排成自然段（无时间轴、无改写、无提炼、无小标题），只做繁转简和对明显识别错误的保守修正。交付的就是"说了什么就记什么"的逐字稿本身；**解析 / 总结 / 问答**不是默认动作，而是在用户提问时基于该逐字稿执行。
...
2. **不改写原文措辞，不做同音错字修正**（错字修正是字幕/语音路线特有的问题）。若文本整段无标点（如从无标点字幕转来的 md），**可以只补中文标点**让其可读——这是合理编辑，但不动措辞、不改字。
3. 按用户需要输出：
...
4. 把 `video.mp4` 当作本地文件交给 `transcribe.sh`（自动走"内嵌字幕 → 无则 Whisper 转写"常规流程）：
   ...
   ```
5. 走统一后处理（繁转简 → 补标点 → 保守错字修正 → 整理为带标点逐字稿段落），交付前问用户入库分类，任务结束清掉 `wechat_video/video.mp4`、`audio.wav` 等大文件（只留文字稿与 metadata）。
...
**标准提取路径（2026-08 实测，易中天 BV1fSgj6uEkR）**——之后遇 B站 链接一律仿照执行：

1. **预检**：`yt-dlp --simulate -f bestaudio/best --print "%(duration_string)s|%(vcodec)s|%(acodec)s|%(filesize_approx)s"`，只看时长/格式/体积，不下载；失败不管（`|| true`）。
2. **字幕轨优先**：`--skip-download --write-subs --write-auto-subs --sub-langs all` 直接抓字幕轨；抓到真实字幕 → `subs2txt.py` 转纯文本 → 结束（不跑语音）。
3. **弹幕排除**：B站 无字幕时常返回 `subs.danmaku.xml`（弹幕，不是字幕）——脚本 `pick_sub` 已过滤弹幕，自动判定"无字幕"。
4. **语音回退**：下载 `bestaudio` 音频轨（5 分钟视频约 5MB，秒下）→ ffmpeg 转 16kHz 单声道 → Whisper 中文转写 → `transcript.txt/.srt`。
5. **后处理**：繁转简 → 补标点（punctuate.py 打底+复核）→ 保守错字修正 → 按说话顺序整理为带标点逐字稿段落 → 交付 `.md` → 清理下载媒体与 WAV。
...
2. **补全中文标点（自动化，一次跑全文）**：转写/字幕文本往往整段无标点，跑流式标点脚本 `python3 <skill_dir>/scripts/punctuate_stream.py <上步文本> <输出> --join`，按**日常语感**补 `。？！，`、合并相邻短句成自然段、承接处大胆并句。**只加标点，不增删内容**；若文本本身标点完整，跳过此步。断句以语义为准，**绝不按 whisper 时间片硬断**。
3. **机器校验 + 少量抽查（效率红线，禁止逐段人工通读重写）**：
   - 默认全程自动化一次跑完；几小时长视频也不例外，人工只抽查全稿开头、结尾与用户点名处（合计 3–5 处），不逐段核对。
   - 修标点（抽查时顺手改）：疑问句→`？`、感叹句→`！`、句尾该断句处→`。`；承接处与弱句尾（的/过/着/得/了）→`，`，去掉明显多余的逗号；能合并的短句按语感并句（如`钟美美是我近些年，特别是做漫谈节目采访过的最年轻的一位嘉宾。`，不写成`采访过的。最年轻的`）；段落按话题/口气切换，不按行切段。
   - 保守修正同音/近音错字：用**常识与上下文**修正，重点是**人名、地名、固定成语、专有词**。举例：`古墓`→`谷牧`、`坑槍`→`踉蹌`、`十一屆三中懸會`→`十一屆三中全會`、`波蘭壯闊`→`波瀾壯闊`、`中美美`→`钟美美`、`贺岗`→`鹤岗`、`博士大学`→`伯克利`、`折球部`→`遮羞布`、`速不相识`→`素不相识`。字幕轨错字通常比语音识别少，但**自动生成字幕（auto-caption）也会有同音/乱序错误**，仍需此步。规则：**只有正确写法十分确定时才改**，保留原句措辞，不增删句子内容。
   - 完整性校验用 `rg` 抽查原 transcript 的首/中/尾关键句是否都在终稿里，不靠通读。
4. **整理为逐字稿（默认交付）**：保留**全部原话与原有顺序**，去掉时间轴与重复的空句/纯占位，把相邻短句按内容节点（话题切换、停顿）并成自然段。**不做**：改写措辞、提炼概括、删减内容、加小标题、补衔接句。产物就是"说了什么就记什么"的逐字稿，且**必须带中文标点**，不能交付整段无标点的文本。
5. 检查逐字稿是否**完整覆盖原话**（与 `transcript.txt` 对照，不能漏句）；确认后作为交付文件另存 `.md`。用户随后就内容提问（总结 / 解析 / 初筛 / 问答）时，基于该逐字稿执行：总结/解析在开头给出要点、正文仍是逐字稿；问答严格依据文本作答（文中没有的说明"文中未提及"）。

> 顺带生成 `transcript.txt`/`transcript.srt`（带时间轴原始转写）作为核对底稿，保留在输出目录，不删除。
...
- **默认输出 = 简体中文逐字稿（即成文文章）**：完整保留全部原话，按说话顺序排成自然段（无时间轴、无小标题、无改写、无提炼），不是 srt 时间轴。交付物本身就是这个逐字稿。
- 只做繁体转简体与"确定无疑"的明显错字修正；不自行添加观点、不概括、不删句、不补衔接。
- **解析 / 总结 / 初筛 / 问答按用户提问触发**：基于交付的逐字稿执行。总结/解析按《初筛摘要格式》在开头给要点；问答严格依据文本（文中没有的说明"文中未提及"）。逐字稿始终是基础交付物。
- 整段纯音乐或与内容无关的占位标记直接剔除。
- 交付时说明本次走的是**字幕轨**还是**语音识别**（语音识别结果更需人工校对）；视频号来源的额外说明它是从本地微信缓存捕获的。
- 交付逐字稿时说明其性质：已是"尽量贴近原话"的整理版，如需逐字核对可对照 `transcript.srt`（带时间轴原样转写）。
