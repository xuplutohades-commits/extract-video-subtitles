...
4. 把 `video.mp4` 当作本地文件交给 `transcribe.sh`（自动走"内嵌字幕 → 无则 Whisper 转写"常规流程）：
   ...
   ```
5. 走统一后处理（繁转简 → 保守错字修正 → 整理为逐字稿段落），交付前问用户入库分类，任务结束清掉 `wechat_video/video.mp4`、`audio.wav` 等大文件（只留文字稿与 metadata）。
...
5. **后处理**：繁转简 → 保守错字修正 → 按说话顺序整理为逐字稿段落 → 交付 `.md` → 清理下载媒体与 WAV。
...
2. **保守修正同音/近音错字**：用**常识与上下文**修正，重点是**人名、地名、固定成语、专有词**。举例：`古墓`→`谷牧`、`坑槍`→`踉蹌`、`十一屆三中懸會`→`十一屆三中全會`、`波蘭壯闊`→`波瀾壯闊`。字幕轨的错字通常比语音识别少，但**自动生成字幕（auto-caption）也会有同音/乱序错误**，仍需此步。规则：**只有正确写法十分确定时才改**，保留原句措辞，不增删句子内容。
3. **整理为逐字稿（默认交付）**：保留**全部原话与原有顺序**，去掉时间轴与重复的空句/纯占位，把相邻短句按内容节点（话题切换、停顿）并成自然段。**不做**：改写措辞、提炼概括、删减内容、加小标题、补衔接句。产物就是"说了什么就记什么"的逐字稿。
4. 检查逐字稿是否**完整覆盖原话**（与 `transcript.txt` 对照，不能漏句）；确认后作为交付文件另存 `.md`。用户随后就内容提问（总结 / 解析 / 初筛 / 问答）时，基于该逐字稿执行：总结/解析在开头给出要点、正文仍是逐字稿；问答严格依据文本作答（文中没有的说明"文中未提及"）。
