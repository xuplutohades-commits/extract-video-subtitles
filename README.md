...
把链接丢给 AI，它就会自动完成：**下载媒体 → 提取音轨 → Whisper 中文识别 → 繁体转简体 → 修正常识性同音错字 → 按内容切段整理成文**，任务结束还会自动清掉占空间的视频/音频，只留文字稿。

## 功能

- 支持常见站点与直链：B站、央视、百度、腾讯、YouTube、mp4/mp3 等
- 高准确率中文识别（Whisper small 模型）
...
- [whisper.cpp](https://github.com/ggerganov/whisper.cpp)（`whisper-cli`，CPU 模式）
...
└── scripts/
    ├── transcribe.sh   # 下载→提音频→Whisper 转写（自动清理大文件）
    └── t2s.py          # 繁体转简体
```

## 说明

多数视频**没有内嵌字幕轨道**，无法直接“下载字幕”，本 skill 走语音识别路径，是这类视频唯一可靠的方案。
